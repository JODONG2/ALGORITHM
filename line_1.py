...
def solution(logs):
    answer = 0
    log_name = ["team_name",":","whgusehd","application_name",":","whgusehd","error_level",":","whgusehd","message",":","whgusehd"]
    ...
    for log in logs:
        ...
        fail = False
        check = 0
        for l,lname in zip(lo,log_name):
            if l == lname :
                check+=1
                continue 
            elif lname =="whgusehd" and l.isalpha():
                continue
            else:
                fail = True 
                break
        if fail or check != 8:
            answer+=1
        #lo = re.split('team_name : | application_name : | error_level : | message : ',log)
