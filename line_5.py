...
def solution(abilities, k):
    ...
    len_abi = len(abilities)
    me = [0 for _ in range(len_abi//2)]
    you = [0 for _ in range(len_abi- len_abi//2)]
    diff = [0 for _ in range(len_abi-len_abi//2)]
    for index in range(len_abi):
        if index%2==0 : 
            you[index//2]=abilities[index]
        else:
            me[index//2] = abilities[index]
            diff[index//2] = you[index//2]-me[index//2]
    if len_abi%2 == 1 : 
        me.append(0)
        diff[-1] = you[-1]-me[-1]
    diff.sort(reverse=True)
    answer = sum(me)+sum(diff[:k])
    return answer