...
def solution(num_teams, remote_tasks, office_tasks, employees):
    ...
    remote_team = [False for _ in range(num_teams)]
    for num,team_work in enumerate(employees):
        team_work = team_work.split()        
        remote = True 
        for work in team_work[1:]: 
            if work in office_tasks: #딕셔너리로 바꾸기
                remote = False 
                break
        if not remote:
            remote_team[int(team_work[0])-1] = True 
        teams[int(team_work[0])-1].append([num,remote])

    for team,remote in zip(teams,remote_team):
        if remote:
            for t in team:
                if t[1]:
                    answer.append(t[0]+1)
